...
from .models import User, Bus, Book, Feedback
...
def list2(request):
    return render(request,'myapp/list2.html')
def thank2(request):
    return render(request,'myapp/thank2.html')
...
def findbus1(request):
    ...
    if request.method == 'POST':
        ...
        bus_list = Bus.objects.filter(source=source_r, dest=dest_r, date=date_r)
        if bus_list:
            return render(request, 'myapp/allbuslist.html', locals())
        else:
            context["error"] = "Sorry no buses available"
            context["error_style"] = "color: white;"  # Set the color of the error message to white
...
def feedback(request):
    ...
    if request.method == 'POST':
        name_r = request.POST.get('name')
        email_r = request.POST.get('email')
        feedback_r = request.POST.get('feedback')
        ...
        return render(request, 'myapp/thank2.html', context)
